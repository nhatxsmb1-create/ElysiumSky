...
public class QuestManager {
    ...
    private void loadQuests() {
        ...
        for (String id : root.getKeys(false)) {
                ...
                for (String key : objSec.getKeys(false)) {
                    ...
                    try {
                        objectives.add(new QuestObjective(
                            ...
                            o.getString("target","STONE"),
                            o.getInt("amount",1)
                        ));
                    } catch (Exception e) {
                        plugin.getLogger().warning("Loi objective " + key + " trong quest " + id);
                    }
                }
            }
            ...
            if (rewSec != null) {
                for (String key : rewSec.getKeys(false)) {
                    ...
                    try {
                        ...
                        plugin.getLogger().warning("Loi reward " + key + " trong quest " + id);
                    }
                }
            }
            ...
            quests.put(id, new Quest(id,
                ...
                objectives, rewards,
                q.getInt("required-level", 0),
                q.getString("required-quest","")
            ));
        }
    }
    ...
    private void completeQuest(Player player, Quest quest, PlayerQuestData data) {
        ...
        for (QuestReward reward : quest.getRewards()) {
            switch (reward.getType()) {
                ...
                case EXP -> {
                    CoreAPI.addExp(player, (long)reward.getAmount());
                    player.sendMessage(ColorUtil.color("  &7+ &a" + (long)reward.getAmount() + " EXP"));
                }
                case ISLAND_POINTS -> {
                    var island = plugin.getIslandManager().getOrCreate(player.getUniqueId(), player.getName());
    ...
    public boolean canAccess(Player player, Quest quest) {
        int level = CoreAPI.getLevel(player);
        if (level < quest.getRequiredLevel()) return false;
